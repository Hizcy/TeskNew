...
public partial class busines_TradeShilist : BasePage//System.Web.UI.Page
{
    public string url = "";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            DataSet ds = Task.BLL.tb_TaskBLL.GetInstance().GetTaskInforByUserTaskId(identity.UserID,taskId);
    ...
    private void BindData()
    {
        if (identity != null)
        {
            string where = " and taskid=" + taskId + " and ActiveType in(2,3) and taskUserId= " + identity.UserID;
            if (status == 1)
            {
                where += " and audit=0";
            }
    ...
    /// <returns></returns>
    public string auditStatus(int index, int audit, int id, string name, int activeType, int buyer, int userid, DateTime addtime, int IsEvaluate)
    {
        DateTime dtime = DateTime.Now;//系统当前时间
        DateTime dt = Convert.ToDateTime(addtime).AddHours(24);//获取数据库的时间
        ...
        if (audit == 0)
        {
            if (IsEvaluate == 0)
            {
                if (activeType == 2)
                {
                    return "<a href=\"javascript:imgdetails(" + userid + "," + taskId + ")\" style=\"color:red\">详情</a><br/><a href='javascript:auditPass(" + id + ",\"" + name + "\"," + activeType + "," + userid + "," + buyer + "," + index + ",\"" + addtime + "\")' ><font color=\"#3DC054\">通过</font></a>"
            ...
            else
            {
                if (dt > dtime)
                {
                    ...
                    {
                        return "<a href=\"Detail.aspx?userid=" + userid + "&taskid=" + taskId + "\" style=\"color:red\">详情</a><br/><a href='javascript:timetqd(" + id + ",\"" + name + "\"," + activeType + "," + userid + "," + buyer + "," + index + ",\"" + addtime + "\")' ><font color=\"#3DC054\">通过</font></a>"
                                + " &nbsp;&nbsp;|&nbsp;&nbsp;<a href='javascript:timetqx(" + id + ",\"" + name + "\"," + activeType + "," + userid + "," + buyer + "," + index + ")' >取消</a>";
                    }
                }
                else
                {
                    ...
                    {
                        return "<a href=\"Detail.aspx?userid=" + userid + "&taskid=" + taskId + "\" style=\"color:red\">详情</a><br/><a href='javascript:auditPass(" + id + ",\"" + name + "\"," + activeType + "," + userid + "," + buyer + "," + index + ",\"" + addtime + "\")' ><font color=\"#3DC054\">通过</font></a>"
                                + " &nbsp;&nbsp;|&nbsp;&nbsp;<a href='javascript:auditRefuse(" + id + ",\"" + name + "\"," + activeType + "," + userid + "," + buyer + "," + index + ")' >取消</a>";
                    }
                }
