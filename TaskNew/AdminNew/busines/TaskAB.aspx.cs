...
using System.Data;
public partial class busines_TaskAB : BasePage //System.Web.UI.Page
{
    ...
    private void BindData()
    {
        if (identity != null)
        {
            ...
            DataSet ds = Task.BLL.tb_TaskBLL.GetInstance().GetAuditTableListByUserId(pager1.PageSize, CurrentPage, " 1=1 and ActiveType =2" + where, out allCount);
    ...
    public string showStatus2(int status)
    {
        ...
        {
            return "<p style=\"color:#3DC054\">待传截图</p>";
        }
        else if (status == 4)
        {
    ...
    /// <returns></returns>
    public string auditStatus(int audit, int id, string name, int activeType, int buyer, int buserid, int evaluation, string taskCode)
    {
        if (identity != null)
        {
            if (audit == 2)
            ...
            {
                return "<p style=\"color:red\">系统取消资格</p>";
            }
            else if (audit == 5)
            {
                if (evaluation == 0)
                {
                    return "<a href=\"javascript:evaluation(" + id + "," + buserid + "," + activeType + "," + taskCode + ")\"><p style=\"color:#3DC054\">待评试客</p></a>";
                }
                else if (evaluation == 1)
                {
                    return "<p style=\"color:#3DC054\">已评论</p>";
                }
            }
            return "";
        }
