...
public partial class CommList : BasePage//System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (!IsPostBack)
            {
                this.Title = "佣金任务专区|淘宝返现平台|折品购";
                this.MetaKeywords = "淘宝返现平台|淘宝佣金平台|淘宝返钱|折品购";
    ...
    private void BindData()
    {
        ...
        DataSet ds = Task.BLL.tb_TaskBLL.GetInstance().GetTaskList(pager1.PageSize, CurrentPage, "  1=1 and ActiveAudit>=1 and ActiveAudit<=2 and ActiveType=1 " + where, out allCount);
        ...
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows != null && ds.Tables[0].Rows.Count > 0)
        {
            ...
            //dv.Sort = "CommissionPrice desc,ActiveEnd desc";
    ...
    //newgoods_div_a2这个是灰色的属性
    public string datetime(string time, int id, int num, int shiyong)
    {
        DateTime dt = DateTime.Now;
        if (dt >= Convert.ToDateTime(time) || shiyong >= num)
        {
            return "<a href=\"" + path + "Details.aspx?id=" + id + "\"onclick=\"javascript:return false;\" class=\"newgoods_div_a\" >已抢完</a>";
        }
        else if (shiyong >= num)
        {
            return "<a href=\"" + path + "Details.aspx?id=" + id + "\"onclick=\"javascript:return false;\" class=\"newgoods_div_a\" >已抢完</a>";
