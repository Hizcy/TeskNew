...
public class UserIdentity : IIdentity 
{
	public UserIdentity(string ticketData)
	{
        ...
        this._guid = ticketArr[4];
	}

    public UserIdentity(string username, string password)
    {
        Task.Entity.tb_UsersEntity user = Task.BLL.tb_UsersBLL.GetInstance().GetModelBy_yong_NamePwd(username, password,0);
        if (user != null)
        {
            ...
            this._guid = System.Guid.NewGuid().ToString().ToUpper();
        }
    }
    ...
    {
        return string.Format("{0}|{1}|{2}|{3}|{4}", this._name, this._userID,this._realName,this._pwd,this._guid);
    }
    ...
    public static bool AuthenticateUser(string username, string password, string role, bool persistLogin)
    {
        try
        {
            ...
            if (user.IsAuthenticated)
            {
                ...
                //----------------------------------------------------------
                Task.BLL.tb_TokenBLL.GetInstance().Delete(user.Name);
                Task.Entity.tb_TokenEntity model = new Task.Entity.tb_TokenEntity();
                model.guid = user.Guid;
                model.sitekey = "";
                model.name = user.Name;
                model.addtime = System.DateTime.Now;
                model.endtime = ticket.Expiration;
                Task.BLL.tb_TokenBLL.GetInstance().Insert(model);
                //----------------------------------------------------------
