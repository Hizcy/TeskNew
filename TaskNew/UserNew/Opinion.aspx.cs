    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (model != null)
            {
                ...
                model.Content = SqlInject(this.txtJianyi.Text.Trim());
                if (identity != null)
                {
                ...
                {
                    model.Mobile = SqlInject(this.txtMobile.Text.Trim());
                    model.UserId = 0;
                }

                int Typeid = int.Parse(SqlInject(ddlType.SelectedValue));
                if (Typeid != 0)
                {
                    model.Type = int.Parse(SqlInject(ddlType.SelectedValue));
