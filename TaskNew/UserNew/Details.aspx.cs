...
public partial class desktop_Details : BasePage//System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            //IList<Task.Entity.tb_TaskEntity> list = Task.BLL.tb_TaskBLL.GetInstance().Gettb_TaskdList().Where(c => c.ActiveType == 1 || c.ActiveType == 2 && c.Addtime < DateTime.Now).ToList();
            //if (list != null && list.Count > 0)
            //{
            //    rptdetails.DataSource = list;
            //    rptdetails.DataBind();
            ...
            if (identity != null)
            {
                ...
                if (listt != null)
                {
                    this.ddlbutry.DataSource = listt;
                    this.ddlbutry.DataTextField = "Buyer";
                    this.ddlbutry.DataValueField = "id";
                    this.ddlbutry.DataBind();
            ...
            try
            {
                Task.Entity.tb_ConcernEntity tmodel = Task.BLL.tb_ConcernBLL.GetInstance().GetConcernModelByUserIdandTaskId(userid, id);
                if (tmodel != null)
                {
        ...
        if (list != null && list.Count > 0)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (!string.IsNullOrEmpty(list[i].Name))
                    sb.Append("<td style=\"width:25%\"><img style=\"width: 80px;border-radius: 50px;height: 80px;\" src=\"" + (string.IsNullOrEmpty(imgUrlReplace((list[i].Header).ToString(),"")) ? "images/touxiang.png" : Image((list[i].Header).ToString(),"s")) + "\"/><p>" + list[i].Name.Substring(0, 1) + "******" + list[i].Name.Substring(list[i].Name.Length - 1, 1) + "</p></td>");
                else
    ...
    private void BindData()
    {
        if (id > 0)
        {
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows != null && ds.Tables[0].Rows.Count > 0)
            {
                ...
                ListImage = imgUrlReplace(ds.Tables[0].Rows[0]["ListImage"].ToString() + "-jpg279", "");
    ...
    public void img()
    {
        ...
        //    Task.Entity.tb_TaskEntity list = Task.BLL.tb_TaskBLL.GetInstance().GetAdminSingle(6);
        //    DirectoryInfo theFolder = new DirectoryInfo(@"C:\details\");
        //    foreach (FileInfo NexFolder in theFolder.GetFiles())
        //    {
        //        digital = Regex.Replace(NexFolder.Name, @"[^\d]*", "");
        //        if (digital == list.Id.ToString())
        //        {
        //            url = NexFolder.Name;
        //        }
        //    }
