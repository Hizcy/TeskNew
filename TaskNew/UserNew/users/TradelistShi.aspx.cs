...
public partial class users_TradelistShi : BasePage//System.Web.UI.Page
{
    ...
    public string show(int audit, string id, int taskId, int Buyer, int activetype)
    {
        if (audit != -20 && audit != -1 && audit != -10 && audit != -2)
        {
            ...
            else if (audit == 3)//上传截图
            {
                ...
                {
                    return "<a href=\"#\" onclick=\"buyjietu(" + id + ")\" style=\"padding:0 0px;color: #f65e91;font-family: 微软雅黑;font-size: 12px;\">上传截图</a>";
                }
            }
    ...
    //立即下单 商家要求 操作记录
    public string showyaoqiu(int audit, int id, int Buyer, int activetype, string qq)
    {
        if (activetype == 3)
        {
            if (audit == 2 || audit == 1 || audit == 3 || audit == 4)
            {
                return "<p><a href=\"http://wpa.qq.com/msgrd?v=3&uin="+qq+"&site=qq&menu=yes\" style=\"color:#0f91b8;\" target=\"_blank\">QQ交谈</a></p><p><a href=\"#\" onclick=operationlog(" + id + ") style=\"color:#888;\">操作记录</a></p>";
            }
            else if (audit == 3 || audit == 4 || audit == 5)
            {
                return "<p><a href=\"#\" onclick=operationlog(" + id + ") style=\"color:#888;\">操作记录</a></p>";
            }
        }
        else
        {
    ...
    public string showStatus(int audit, int id, string img)
    {
        ...
        {
            return "<a href='#' onclick=\"fdimg(" + id + ",'" + img + "'," + audit + ")\"><img src=\"" + imgUrlReplace(img.ToString()+"-jpg279","") + "\" style=\"width:50px;height:50px;\" onerror=\"this.src='../images/touxt.jpg';\" /></a>";
        }
        else
        {
            return "--";
