        ...
        <div class="ccbox ccboxup clearfix ">
           ...
           <div class="fl">
        	...
        	<div class="fl" style="width:695px;">
                ...
                <div class="m-s2-box">
                    <h4>1.如图示，进入店铺首页：</h4>
                    <div class="s2-box-pic"><img id="imgwid" src="<%=listImage %>" onerror="../images/buery-make-img5.jpg" alt="pic"/></div>
...
<div class="m-wrap clearfix">
    ...
    <script> 
        $(document).ready(function () {
            var flagNumber = 0;
            $(".js4").click(function () {
                splitStr($.trim($("#s1").val()));
                if (flagNumber===0) {
                    $("#msg").html('<div style="color:red;: margin:20px 30px;  display:inline!important;">第一个链接属于非法链接！</div>');
                    return;
                }
                else {
                    flagNumber = 0;
                }
                splitStr($.trim($("#s2").val()));
                if (flagNumber === 0) {
                    $("#msg").html('<div style="color:red;: margin:20px 30px;  display:inline!important;">第二个链接属于非法链接！</div>');
                    return;
                }
                else {
                    flagNumber = 0;
                }
                splitStr($.trim($("#s3").val()));
                if (flagNumber === 0) {
                    $("#msg").html('<div style="color:red;: margin:20px 30px;  display:inline!important;">第三个链接属于非法链接！！</div>');
                    return;
                }
                else {
                    flagNumber = 0;
                }
                splitStr($.trim($("#s4").val()));
                if (flagNumber === 0) {
                    $("#msg").html('<div style="color:red;: margin:20px 30px;  display:inline!important;">第四个链接属于非法链接！非法链接！</div>');
                    return;
                }
                else {
                    flagNumber = 0;
                ...
                var browseProduct1 = $.trim($("#s1").val()); 
                var browseProduct2 = $.trim($("#s2").val());
                var browseProduct3 = $.trim($("#s3").val());
                var browseProduct4 = $.trim($("#s4").val());
                $.ajax({
                    url: '../data/data.aspx',
                    type: 'POST',
                    data: {
