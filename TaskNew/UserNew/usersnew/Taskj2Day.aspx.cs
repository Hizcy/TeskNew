...
public partial class usersnew_Taskj2Day : BasePage//System.Web.UI.Page
{
    ...
    public int task_UserId //接手信息id  tb_task_userid id
    {
        get
        {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            //获取买家名字
            Task.Entity.tb_BuyerEntity ds = Task.BLL.tb_BuyerBLL.GetInstance().GetAdminSingle(buyerId);
            if (ds != null)
            {
                buyers = ds.Buyer;
            }
            //获取时间
            Task.Entity.tb_Task_UserEntity user = Task.BLL.tb_Task_UserBLL.GetInstance().GetAdminSingle(task_UserId);
            if (user != null)
            {
