...
using System.Data;
public partial class admin_AuditTask : BasePage//System.Web.UI.Page
{
    ...
    public int state//获取当前选中的状态 判断商品上架还是下架 
    {
        get
        {
            object obj = Request.QueryString["state"];
            if (obj == null)
            {
                return 1;
            }
    ...
    private void BindData()
    {
        if (identity != null)
        {
            ...
            if (state == 1)
            {
                where += "and ActiveAudit>=1 and ActiveEnd >'" + DateTime.Now+"'";
            }
            else if (state == 2)
            {
                where += " and ActiveAudit < 1 and ActiveEnd <'" + DateTime.Now+"'";
            ...
            DataSet ds = Task.BLL.tb_TaskBLL.GetInstance().GetTaskInfor(pager1.PageSize, CurrentPage, " 1=1 and activetype=3 and isEvaluate=0  " + where, out allCount);
    ...
    public string showData(int audit, int id, decimal money, decimal GuaranteePrice, string TaskCode, int ActiveType)
    {
        ...
        //{
        //    return "<p><a href=\"AuditShilist.aspx?id=" + id + "\" target=\"_blank\" style=\"color:#fe4800;\">资格审核</a></p><p><a href=\"AuditTaskShi.aspx?type=" + id + "\" target=\"_blank\" style=\"color:#3DC054;\">管理订单</a></p>";
        //}
        else if (audit != 0 && audit != -1 && ActiveType == 3)
        {
            return "<p><a href=\"AuditShilist.aspx?id=" + id + "\" target=\"_blank\" style=\"color:#fe4800;\">资格审核</a></p>";
        }
        else if (audit == -1)
