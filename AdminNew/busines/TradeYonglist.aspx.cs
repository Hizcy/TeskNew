...
public partial class busines_TradeYonglist : BasePage//System.Web.UI.Page
{
    public string url = "";
    ...
    private void BindData()
    {
        if (identity != null)
        {
            string where = " and taskid=" + taskId + " and ActiveType=1 and taskUserId= " + identity.UserID;
            if (status == 1)
            {
                where += " and audit=0";
            }
    ...
    public string auditStatus(int index, int audit, int id, string name, int activeType, int buyer, int userid)
    {
        if (audit == 0)
        {
            ...
            return "<span style=\"color:#FC3C75\">商家取消</span>";
        else if (audit == -20)
            return "<span style=\"color:#FC3C75\">系统自动取消</span>";
