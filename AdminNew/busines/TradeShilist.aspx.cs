...
public partial class busines_TradeShilist : BasePage//System.Web.UI.Page
{
    ...
    public string auditStatus(int index, int audit, int id, string name, int activeType, int buyer, int userid)
    {
        if (audit == 0)
        {
            if (activeType == 2)
            {
                return "<a href=\"javascript:imgdetails(" + userid + "," + taskId + ")\" style=\"color:red\">详情</a><br/><a href='javascript:auditPass(" + id + ",\"" + name + "\"," + activeType + "," + userid + "," + buyer + "," + index + ")' ><font color=\"#3DC054\">通过</font></a>"
