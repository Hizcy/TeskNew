...
public partial class user_TaskShi :BasePage //System.Web.UI.Page
{
    ...
    private void BindData()
    {
        if (identity != null)
        {
            ...
            DataSet ds = Task.BLL.tb_TaskBLL.GetInstance().GetAuditTableListByUserId(pager1.PageSize, CurrentPage, " 1=1 and ActiveType >=2 and ActiveType<=3 " + where, out allCount);
    ...
    /// <returns></returns>
    public string auditStatus(int audit, int id, string name, int activeType, int buyer, int buserid, int evaluation)
    {
        if (identity != null)
        {
            if (audit == 2)
            ...
            else if (audit == 5)
            {
                if (evaluation == 0)
                {
                    return "<a href=\"javascript:evaluation("+id+","+buserid+")\"><p style=\"color:#3DC054\">待评试客</p></a>";
                }
