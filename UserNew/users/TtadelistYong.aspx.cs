...
public partial class users_TtadelistYong : BasePage//System.Web.UI.Page
{
    public int audit
    {
        get
    ...
    private void BindData()
    {
        ...
        else
        {
            ...
            if (audit == 6)
            {
                where += " and audit in (-1,-10,0)";
            } 
        }
        int allCount;
        int CurrentPage = 0;
        CurrentPage = this.pager1.CurrentPageIndex;
        DataSet ds = Task.BLL.tb_TaskBLL.GetInstance().GetBuyerJieShouTaskList(pager1.PageSize, CurrentPage, " 1=1 and activetype=1 " + where, out allCount);
    ...
    public string show(int audit, string id)
    {
        if (audit != -20 && audit != -1 && audit != -10 && audit != -2)
        {
            ...
            {
                return "<input type=\"button\" class=\"task_2_th1_dingdan\" value=\"上传截图\" onclick=\"buyjietu(" + id + ")\" style=\"padding:0 10px;color: #f65e91;font-family: 微软雅黑;font-size: 12px;\"/>";
            }
