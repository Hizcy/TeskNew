...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
                        ...
                        <div class="newgoods-wrap one-tmall">
                           <div class="abs"><p><%#activemethodmf(int.Parse(Eval("ActiveMethod").ToString()),int.Parse(Eval("id").ToString())) %></p></div>
                            ...
                            <a href="<%=path %>Details.aspx?id=<%#Eval("Id") %>" class="piwik_link one-brand-hd" target="_blank" >
                                <img style="width:240px;height:235px; background-repeat: no-repeat;background-position: center;" class="lazy" src="<%=path %>images2/quesheng.jpg" data-original="<%#Image(Eval("ProductImage").ToString(),"s") %>"></img>
        			            
                            </a>
                            <p class="newgoods-pai zoom">
        	                    <span style="font-size:16px;"><%#Eval("ActiveName").ToString().Length>10?Eval("ActiveName").ToString().Replace("%20","").Substring(0,8)+"…":Eval("ActiveName").ToString().Replace("%20","") %> </span>                       
        			        ...
        			        <p class="newgoods-pai zoom">
        	                    <span class="newgoods-price">￥<%#money(Eval("Price").ToString()) %></span>
                               <%-- <span class="newgoods-pricef">返还：￥<%#Convert.ToDecimal(Eval("Price").ToString())+Convert.ToDecimal(Eval("CommissionPrice").ToString()) %> </span>--%>
                                 <span class="newgoods-time" style="margin-left:10px;float:right;margin-top:5px">剩余份数：<%#int.Parse(Eval("ProductNum").ToString())-int.Parse(Eval("shiyong").ToString()) %>/<%#Eval("ProductNum") %></span>                                   
        	                </p>
                           <%-- <p class="newgoods-pai zoom">
    ...
    <script type="text/javascript">
        ...
        function ($) {
            $("img.lazy").lazyload({
                placeholder: "<%=path %>images2/quesheng.jpg",
                effect: "fadeIn",
                threshold: 200
            });
        });
