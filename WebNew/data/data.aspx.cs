...
public partial class data_data : BasePage//System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            switch (type)
            {
                ...
                case "delnotice"://删除公告
                        ...
                        break;
                case "addnotice"://添加公告
                        addnotice(Request.Form["title"].ToString(), Request.Form["content"].ToString(), int.Parse(Request.Form["bulletin"].ToString()));
                        break;
                case "updatenotice"://修改公告
                        updatenotice(Request.Form["title"].ToString(), Request.Form["content"].ToString());
                        break;
                case "daudit"://代审试用任务
    ...
    /// 
    System.Text.StringBuilder sb = new System.Text.StringBuilder();
    public void addnotice(string title, string content, int bulletin) 
    {
        try
        {
            Jnwf.Utils.Log.Logger.Log4Net.Error("data,addnotice1,title:" + title + ",content:" + content + "");
